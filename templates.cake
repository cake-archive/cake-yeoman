...
.Does(() => {
  GitClone(
    ...
    "./.templates",
    EnvironmentVariable("GITHUB_USER"),     // these can be taken out when the repo is made public
    EnvironmentVariable("GITHUB_PASSWORD")  // ^^
    );
});
...
void CleanTemplatesDirectory(DirectoryPath templateDirectoryPath) {
  ...
  DeleteDirectory(templateDirectoryPath, true);
}